...
//Ked mam 64 bit compiler tak vidim trocha lepsie v akom fily to pada
// TU je packageing project kde sa makne apk svieti lebo to je target co sa vlasne builduje
//LOOK
typedef struct {
	EGLDisplay* window;
	...
	char db_folder_path[MAX_PATH_LENGTH];
...
void on_key(int key, int scancode, int action, int mods) {
	...
	int control = 0;
	int exclusive = craft_get_input_mode();
...
void craft_get_cursor_pos(double* xpos, double* ypos)
{
	*xpos = g_xpos;
	*ypos = g_ypos;
}
...
int input_mode = 0;
void craft_set_input_mode(int exclusive)
{
	input_mode = exclusive;
}

int craft_get_input_mode() {

	return input_mode;
}


void on_mouse_button(double x, double y,  int action) {

	if (action != AMOTION_EVENT_ACTION_DOWN) {
		if (action == AMOTION_EVENT_ACTION_UP) {
			craft_set_input_mode(0);
		}
		return;
	}
	else {
		craft_get_cursor_pos(&px, &py);
	}

	craft_set_input_mode(1);
}
...
void handle_movement(double dt) {
	...
	if (!g->typing) {
		...
		if (arrow_state == 2 /* forward */ ) sz--; 
		if (arrow_state == 3 /* backward */) sz++;
		if (arrow_state == 0 /* left */) sx--;
		if (arrow_state == 1 /* right */) sx++;
		/* if (glfwGetKey(g->window, GLFW_KEY_LEFT)) s->rx -= m;
		if (glfwGetKey(g->window, GLFW_KEY_RIGHT)) s->rx += m;
		if (glfwGetKey(g->window, GLFW_KEY_UP)) s->ry += m;
		if (glfwGetKey(g->window, GLFW_KEY_DOWN)) s->ry -= m; */
...
void parse_buffer(char* buffer) {
	...
	while (line) {
		...
		if (sscanf(line, "E,%lf,%d", &elapsed, &day_length) == 2) {
			craft_set_time(fmod(elapsed, day_length));
...
void reset_model() {
	...
	g->message_index = 0;
	g->day_length = DAY_LENGTH;
	craft_set_time(g->day_length / 3.0);
	g->time_changed = 1;
}
...
static int engine_init_display(struct engine* engine) {
	...
	* the first EGLConfig that matches our criteria */
	...
	EGLConfig *supportedConfigs = malloc(sizeof(EGLConfig) * numConfigs);
	...
	* ANativeWindow buffers to match, using EGL_NATIVE_VISUAL_ID. */
	...
	load_png_texture(engine->app->activity->assetManager, "textures/texture.png");
	...
	load_png_texture(engine->app->activity->assetManager, "textures/font.png");
	...
	load_png_texture(engine->app->activity->assetManager, "textures/sky.png");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	load_png_texture(engine->app->activity->assetManager, "textures/sign.png");

	GLuint program;

	program = load_program(engine->app->activity->assetManager,
	...
	{
		g->mode = MODE_OFFLINE;
		char internalPath[MAX_PATH_LENGTH];
		strcpy(internalPath, engine->app->activity->internalDataPath);
		strcat(internalPath, "/");
		snprintf(g->db_folder_path, MAX_PATH_LENGTH, "%s", internalPath);

		strcat(internalPath, DB_PATH);
		snprintf(g->db_path, MAX_PATH_LENGTH, "%s", internalPath);
	...
	if (g->mode == MODE_OFFLINE || USE_CACHE) {
		db_enable();
		if (db_init(g->db_path, g->db_folder_path)) {
			return -1;
		}
	...
	reset_model();
	last_commit = craft_get_time();
	last_update = craft_get_time();
...
static void engine_draw_frame(struct engine* engine) {
	...
	g->scale = 1;
	eglQuerySurface(engine->display, engine->surface, EGL_WIDTH, &g->width);
	eglQuerySurface(engine->display, engine->surface, EGL_HEIGHT, &g->height);
...
static void engine_term_display(struct engine* engine) {
	...
	// SHUTDOWN //
	db_save_state(g->players->state.x, g->players->state.y, g->players->state.z, g->players->state.rx, g->players->state.ry);
	db_close();
	db_disable();
        /*client_stop();
        client_disable();*/
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
	...
	if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
		...
		//on_mouse_button(engine, AMotionEvent_getAction(event));
		return 1;
	}
	return 0;
}
...
JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_onArrow(JNIEnv* env, jobject thiz, jint arrowId)
{
	arrow_state = arrowId;
}

JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_onButtonClick(JNIEnv* env, jobject thiz, jint buttonId)
{
	if (buttonId == 0) // remove button
	{
		on_left_click();
	}
	else if (buttonId == 1) // add button
	{
		on_right_click();
	}
	else if (buttonId == 2) // fly button
	{
		g->flying = !g->flying;
	}
	else if (buttonId == 3) //change texture
	{
		g->item_index = (g->item_index + 1) % item_count;
	}
}

JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_onTouch(JNIEnv* env, jobject thiz, jfloat jx, jfloat jy, jint action)
{
	g_xpos = jx;
	g_ypos = jy;
	on_mouse_button(jx, jy, action);
}

JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_init(JNIEnv* env, jobject thiz, jobject jassetManager, jstring jinternalPath)
{
	finishedSetup = false;
	AAssetManager* assetManager = AAssetManager_fromJava(env, jassetManager);
	assert(NULL != assetManager);

	const char *nativeInternalPath = (*env)->GetStringUTFChars(env, jinternalPath, 0);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	load_png_texture(assetManager, "textures/texture.png");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	load_png_texture(assetManager, "textures/font.png");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	load_png_texture(assetManager, "textures/sky.png");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	load_png_texture(assetManager, "textures/sign.png");

	GLuint program;

	program = load_program(assetManager, "shaders/block_vertex.glsl", "shaders/block_fragment.glsl");
	block_attrib.program = program;
	block_attrib.position = glGetAttribLocation(program, "position");
	block_attrib.normal = glGetAttribLocation(program, "normal");
	block_attrib.uv = glGetAttribLocation(program, "uv");
	...
	program = load_program(assetManager, "shaders/line_vertex.glsl", "shaders/line_fragment.glsl");
	line_attrib.program = program;
	line_attrib.position = glGetAttribLocation(program, "position");
	line_attrib.matrix = glGetUniformLocation(program, "matrix");

	program = load_program(assetManager, "shaders/text_vertex.glsl", "shaders/text_fragment.glsl");
	text_attrib.program = program;
	text_attrib.position = glGetAttribLocation(program, "position");
	text_attrib.uv = glGetAttribLocation(program, "uv");
	text_attrib.matrix = glGetUniformLocation(program, "matrix");
	...
	program = load_program(assetManager, "shaders/sky_vertex.glsl", "shaders/sky_fragment.glsl");
	sky_attrib.program = program;
	sky_attrib.position = glGetAttribLocation(program, "position");
	sky_attrib.normal = glGetAttribLocation(program, "normal");
	sky_attrib.uv = glGetAttribLocation(program, "uv");
	...
	//else 
	{
		g->mode = MODE_OFFLINE;
		char internalPath[MAX_PATH_LENGTH];
		strcpy(internalPath, nativeInternalPath);
		strcat(internalPath, "/");
		snprintf(g->db_folder_path, MAX_PATH_LENGTH, "%s", internalPath);
	...
	(*env)->ReleaseStringUTFChars(env, jinternalPath, nativeInternalPath);
...
JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_terminate(JNIEnv* env, jobject thiz)
{
	if (!finishedSetup)
		return;
...
JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_surfaceChanged(JNIEnv* env, jobject thiz, jint width, jint height)
{
	// LOCAL VARIABLES //
	memset(&fps, 0, sizeof(FPS));
	reset_model();
...
JNIEXPORT void JNICALL Java_com_Craft_1Java_NativeLib_draw(JNIEnv* env, jobject thiz)
{
	// WINDOW SIZE AND SCALE //
	//g->scale = get_scale_factor();
	//glfwGetFramebufferSize(g->window, &g->width, &g->height);
